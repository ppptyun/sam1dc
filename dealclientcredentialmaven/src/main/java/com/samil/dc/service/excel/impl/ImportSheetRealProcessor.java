...
/**
 * 마이그레이션용_부동산자문 Sheet 처리 클래스
 *
 */
public class ImportSheetRealProcessor extends AbstractSheetProcessor {
	
	private List<ImportRealBean> list = new ArrayList<ImportRealBean>();

	public ImportSheetRealProcessor(DBConnection connection, Sheet sheet) throws ExcelProcessException {
	...
	public AbstractSheetProcessor ready() throws ExcelProcessException {
		...
		*/
		columns.add(new DefColumn("target.dealnm", "Target > 거래대상"));
	...
	public AbstractSheetProcessor parse() throws ExcelProcessException {
		for (int r = ExcelHelper.ROW_INDEX_START; r < sheet.getRows(); r++) {
			...
			ImportRealBean project = new ImportRealBean(r, ExcelHelper.PROJECT_REFYEARLY, prjtcd);
	...
	public AbstractSheetProcessor save() throws ExcelProcessException {
		...
		try {
			for (ImportRealBean project : list) {
				if (!project.isValid()) {
					continue;
				}
				...
				if (!exist) {
					...
					// League Table Real 정보 등록
					dao.sqlInsertLeagueTableSub(project);
				}
				
				// League Table Target 정보 저장
				ImportBizBean target = project.getTarget();
				if (!StringUtils.isBlank(target.getDealnm())) {
					target.setActcd(Constants.Actor.REAL_TARGET);
				...
				if (!StringUtils.isBlank(seller.getComphannm())) {
					seller.setActcd(Constants.Actor.REAL_SELLER);
				...
				if (!StringUtils.isBlank(sellfinance.getComphannm())) {
					sellfinance.setActcd(Constants.Actor.REAL_SELLER);
				...
				if (!StringUtils.isBlank(sellaudit.getComphannm())) {
					sellaudit.setActcd(Constants.Actor.REAL_SELLER);
				...
				if (!StringUtils.isBlank(selllaw.getComphannm())) {
					selllaw.setActcd(Constants.Actor.REAL_SELLER);
				...
				if (!StringUtils.isBlank(buyer.getComphannm())) {
					buyer.setActcd(Constants.Actor.REAL_BUYER);
				...
				if (!StringUtils.isBlank(buyfinance.getComphannm())) {
					buyfinance.setActcd(Constants.Actor.REAL_BUYER);
				...
				if (!StringUtils.isBlank(buyaudit.getComphannm())) {
					buyaudit.setActcd(Constants.Actor.REAL_BUYER);
				...
				if (!StringUtils.isBlank(buylaw.getComphannm())) {
					buylaw.setActcd(Constants.Actor.REAL_BUYER);
