var DetailLT = (function(){
	...
	var cal3 = "";
	var cal4 = "";
	var modifycheck = false; // 수정이 이루어졌을 경우, 리스트로 전환 시 데이터 재로드하기 위함.
	...
	function doLoadDetailLeagueTable(){
		...
		Ajax.post(url, {prjtcd:lv_data.prjtcd}, function(serverData){
			if(serverData.result) {
				$.extend(lv_data.prjtinfo, serverData.data.project);
	...
	function drawDetailLeagueTable(){
		var _h = [];
		var _data = $.extend({}, lv_data.prjtinfo);
		...
		if(lv_data.condition == CONST.CONDITION.READ) {
			...
			if(lv_data.type == CONST.TYPE.BUY) {
				...
				var deal1nm = []; for(var i=0; i<_data.deal1.length; i++){deal1nm.push(_data.deal1[i].dealitemnm);};
				$("#deal1Type").val(deal1nm.join(" / "));
			...
			var tmpList = $.extend([], __cons_type_list);
			for(var i=0; i<tmpList.length; i++){
				if(tmpList[i].code == CONST.conscd.UNCLF){
					tmpList.splice(i, 1);
					break;
				}
			}
			$("#conscd").empty().append(getHtmlSelection("consType", tmpList, _data.conscd));
			$("#statuscd").empty().append(getHtmlSelection("statusType", __status_list, _data.statuscd));
			$("#confcd").empty().append(getHtmlSelection("confType", __conf_type_list, _data.confcd));
			$("#ratecd").empty().append(getHtmlSelection("rateType", __curr_list, _data.currcd));
			$("#borddeal").empty().append(getHtmlSelection("borddealType", __bord_type_list, _data.borddealcd));
			
			if(lv_data.type == CONST.TYPE.BUY) {
				$("#buyoutcd").empty().append(getHtmlSelection("buyoutType", __buyout_type_list, _data.buyoutcd));
				_h = [];
				...
				$("#deal2").empty().append(getHtmlSelection("deal2Type", __buy_deal_type2_list, _data.dealcd2));
				$("#deal3").empty().append(getHtmlSelection("deal3Type", __buy_deal_type3_list, _data.dealcd3));
			} else if(lv_data.type == CONST.TYPE.MNA) {
				$("#deal1").empty().append(getHtmlSelection("deal1Type", __mna_deal_type1_list, _data.dealcd1));
				$("#deal2").empty().append(getHtmlSelection("deal2Type", __mna_deal_type2_list, _data.dealcd2));
				$("#deal3").empty().append(getHtmlSelection("deal3Type", __mna_deal_type3_list, _data.dealcd3));
			}
		}
	...
	function getHtmlSelection(id, list, value){
		_h = [];
		_h.push('<select id="' + id + '" class="inp_change"><option value="">선택해주세요.</option>');
		for(var i=0; i<list.length; i++){
			_h.push('<option value="' + list[i].code + '" '+ ((list[i].code == value)?"selected":"") + ' >' + list[i].name + '</option>');
		}
		_h.push('</select>');
		return _h.join("");
	}
	...
	function editEventSetting() {
		$("#consType").change(function(){
			...
			if(conscd == CONST.conscd.UNCLF || conscd == "") {
				...
				$("#consType").val(lv_data.prjtinfo.conscd);
				return false;
			}
			...
			dhtmlx.confirm({
				...
				callback:function(result){
					if(result) {
						...
						param.befconscd = lv_data.prjtinfo.conscd;
		...
		function inputDataDelete() {
			...
			if(_s.hasClass("consCompanyKorNm")){
				var $charger = $(this).closest("tr").find(".charger ");
				if($.trim($charger.val()) != "" || $charger.data("code") != "" ){
					if(confirm("자문사를 삭제할 경우 담당자 정보가 초기화 됩니다.\n\n계속 하시겠습니까?")){
						$charger.val("");
						$charger.data("code", "");
	...
	function bindEvent(){
		...
		$("#btn_detail_chrgconf").off("click").on("click", function(){
			var typecd = "";
			if(lv_data.prjtinfo.chrgconfcd == CONST.chrgconfcd.YES) {
				if(!confirm("해제됩니다.")) {
					return false;
				}
				typecd = CONST.chrgconfcd.NO;
			} else if(lv_data.prjtinfo.chrgconfcd == CONST.chrgconfcd.NO) {
				alert("확인됩니다.");
		...
		$("#btn_detail_modify").off("click").on("click", function(){
			...
			if(!editAuth){
				if(!editAuth && (lv_data.prjtinfo.statuscd == CONST.statuscd.COMPLETE || lv_data.prjtinfo.statuscd == CONST.statuscd.DROP)) {
		...
		$(document).off('click', '.btn_conscompany_sel').on('click', '.btn_conscompany_sel', function() {
			var _s = $(this);
			var $input = _s.parent().find("input");
			var param = {};
			param.type = "";
			param.compcd = $input.data("code");
			...
			POPUP.ViewPopup("consCompanySearch", param, function(selectedConsCompanyInfo) {
				if(selectedConsCompanyInfo.hangnm != "" && selectedConsCompanyInfo.hangnm != null) {
					...
					if(selectedConsCompanyInfo.isChanged){
						if(confirm("자문사가 변경되어 담당자 정보가 초기화 됩니다.\n\n계속 하시겠습니까?")){
							$charger.data("code", "");
							$charger.val("");	
						}else{
							return;
		...
		$(document).off('click', '.btn_charge_sel').on('click', '.btn_charge_sel', function() {	
			var _s = $(this);
			var $info = _s.parent().find("input");
			POPUP.ViewPopup("userSearch", null, function(_userInfo) {
				$info.data("code", _userInfo.emplno);
				$info.val(_userInfo.kornm);
			});
		})
	...
	function drawSubstructureHtml() {
		...
		_h.push('<h3 class="dc_con_title">기본정보<div class="dc_con_title_date"><span class="required">필수 기재 항목　</span> 최종수정일 : <span id="upddt"></span></div></h3>');
	...
	function drawBasicHtml() {
		...
		if(lv_data.type == CONST.TYPE.BUY) {
			_h.push('<th><span class="required"></span>Buyout / Non-Buyout</th>');
	...
	function drawConsCompanyHtml(classid, kind, data) {
		...
		do {
			...
			_h.push('<td>');
			_h.push('<div class="data_cell">');
			_h.push('<input type="text" maxlength="650" readonly="readonly" class="essenValue consCompanyKorNm inp_change" ' 
					+ ' value="' + (data[i].comphannm ? data[i].comphannm : "") + '" ' 
					+ ' data-code="' + (data[i].compcd ? data[i].compcd : "") + '" ' 
					+ ' data-seq="' + (data[i].seq ? data[i].seq : "") + '" ' 
					+ ' data-actcd="' + (data[i].actcd ? data[i].actcd : "") + '" ' 
					+ ' data-bizcd="' + (data[i].bizcd ? data[i].bizcd : "") + '"/>');
			_h.push('<button type="button" class="btn ico_delete modify_btn btn_delete_sel" title="삭제">삭제</button>');
			...
			_h.push('<input type="text" maxlength="30" class="inp_change charger ' + (data[i].compcd == "1"?'':'modify_inp') + '" ' 
					+ ' value="' + (data[i].chrgempnm1 ? data[i].chrgempnm1 : "")  + '" '
					+ ' data-code="' + (data[i].chrgempno1 ? data[i].chrgempno1 : "") + '" ' 
					+ (data[i].compcd == "1"?' readonly="readonly" ':'') + ' />');
			_h.push('<button type="button" class="btn ico_delete btn_delete_sel '+ (data[i].compcd == "1"?'modify_btn':'') +'" title="삭제" style="display:' + (data[i].compcd == "1"?"block":"none") + '">삭제</button>');
			_h.push('<button type="button" class="btn_charge_sel btn btn_red '+ (data[i].compcd == "1"?'modify_btn':'') +'" style="display:' + (data[i].compcd == "1"?"block":"none") + '">선택</button>');
			_h.push('</div></td>');
			_h.push('</tr>');
			i++;
		} while(i<data.length);
	...
	function drawPlusConsCompanyHtml(classid, kind) {
		...
		_h.push('<button type="button" class="btn ico_delete modify_btn btn_delete_sel" title="삭제" style="display:none">삭제</button>');
		_h.push('<button type="button" class="btn_charge_sel btn btn_red modify_btn" style="display:none">선택</button>');
		_h.push('</div></td>');
		_h.push('</tr>');
		return _h;
	}
	...
	*/
	...
	function drawCurrHtml(data) {
		...
		if(lv_data.type == CONST.TYPE.REAL) {
			...
			_h.push('<th><span class="required"></span>거래형태 1</th>');
		...
		if(lv_data.type == CONST.TYPE.REAL) {
			...
			_h.push('<th><span class="required"></span>거래형태 2</th>');
		...
		if(lv_data.type == CONST.TYPE.REAL) {
			...
			_h.push('<th><span class="required"></span>거래형태 3</th>');
