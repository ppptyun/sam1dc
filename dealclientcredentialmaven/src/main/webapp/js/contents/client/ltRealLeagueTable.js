var GRID_OBJ = (function(){
	...
	var m_gridDefinition = {
			...
			columns:[  
						{id:"chrgconfcd", label:["본부 담당자 확인", "확인 여부"], type:"coro", align:"center", valign:"middle", width:"80", sort: "str"},
			         	...
			         	{id:"tdealnm", label:["Target", "#rspan"], type:"ro", align:"left", valign:"middle", width:"200", sort: "str"},
	...
	function doInit(){
		...
		if(!m_gridObj){
			...
			/*m_gridObj.setInitWidthsP("8,8,6,10,15,10,5,5,5,8,15,10,10,10,10,10");
	...
	function bindEvent(){
		...
		$("#btn_excel_download").off("click").on("click", function() {
			var param = {};
			var excelParam = {};
			excelParam.filename = "부동산자문_"+ SAMILCOMMON.toTimeString(new Date);
			excelParam.column 	= DOWNLOAD_COLUMN_DEF.REALLEAGUE;
			param.SystemCode 	= "DcExcelDownloadDAO";
			param.Method 		= "sqlSelectRealLeagueTable";
	...
	function doLoadLeagueTable(){
		...
		SAMILCOMMON.progressOn();
		Ajax.post("LtConsultRealList", {}, function(serverData){
			if (serverData.result) {
