var GRID_OBJ = (function(){
	...
	var m_gridDefinition = {
			...
			skin:"dhx_skyblue",
			columns:[  
						{id:"chrgconfcd", label:["본부 담당자 확인", "확인 여부"], type:"coro", align:"center", valign:"middle", width:"80", sort: "str"},
			         	...
			         	{id:"scomphannm", label:["합병(존속/신설)법인", "기업명"], type:"ro", align:"left", valign:"middle", width:"200", sort: "str"},
			         	{id:"sindunm", label:["#cspan", "Industry"], type:"ro", align:"left", valign:"middle", width:"200", sort: "str"},
			         	{id:"bcomphannm", label:["피합병(소멸)법인", "기업명"], type:"ro", align:"left", valign:"middle", width:"200", sort: "str"},
	...
	function doInit(){
		...
		if(!m_gridObj){
			...
			/*m_gridObj.setInitWidthsP("8,8,6,10,15,10,5,5,5,8,10,10,10,10,10");
			...
			var bgColor = [];
			for(var i=0; i<m_gridDefinition.columns.length; i++){
				if(SAMILCOMMON.intersection(SAMILCOMMON.getArrayByColumn(USERROLE,["rolecd"]), editableColumns[m_gridDefinition.columns[i].id]).length > 0){
					bgColor[i] = "#f6dabf";
				}else{
					bgColor[i] = "";
				}
	...
	function bindEvent(){
		...
		$("#btn_excel_download").off("click").on("click", function() {
			var param = {};
			var excelParam = {};
			excelParam.filename = "합병분할자문_"+ SAMILCOMMON.toTimeString(new Date);
			excelParam.column 	= DOWNLOAD_COLUMN_DEF.MNALEAGUE;
			param.SystemCode 	= "DcExcelDownloadDAO";
			param.Method 		= "sqlSelectMnaLeagueTable";
	...
	function doLoadLeagueTable(){
		...
		SAMILCOMMON.progressOn();
		Ajax.post("LtConsultMnaList", {}, function(serverData){
			if (serverData.result) {
