...
public class ExportExcelSatPrjtInfoV3 extends AbstractExportExcel {
	
	public ExportExcelSatPrjtInfoV3(Map<String, Object> option) throws Exception {
		super(option);
	}
	...
	public void ready() throws Exception {
		...
		columns.add(new DefExportColumn("chargptrNm", Arrays.asList("#cspan", "EL", "이름"), basicStyle));
		columns.add(new DefExportColumn("chargptr", Arrays.asList("#cspan", "#cspan", "사번"), textCenterStyle));
		columns.add(new DefExportColumn("chargmgrNm", Arrays.asList("#cspan", "PM", "이름"), basicStyle));
		columns.add(new DefExportColumn("chargmgr", Arrays.asList("#cspan", "#cspan", "사번"), textCenterStyle));
		columns.add(new DefExportColumn("rcrdMgrNm", Arrays.asList("#cspan", "Record Manager1", "이름"), basicStyle));
		columns.add(new DefExportColumn("rcrdMgr", Arrays.asList("#cspan", "#cspan", "사번"), textCenterStyle));
		columns.add(new DefExportColumn("rcrdMgrNm2", Arrays.asList("#cspan", "Record Manager2", "이름"), basicStyle));
		columns.add(new DefExportColumn("rcrdMgr2", Arrays.asList("#cspan", "#cspan", "사번"), textCenterStyle));
		columns.add(new DefExportColumn("rcrdMgrNm3", Arrays.asList("#cspan", "Record Manager3", "이름"), basicStyle));
		columns.add(new DefExportColumn("rcrdMgr3", Arrays.asList("#cspan", "#cspan", "사번"), textCenterStyle));
		...
		columns.add(new DefExportColumn("bizFrdt", Arrays.asList("#cspan", "사업연도", "시작월"), textCenterStyle));
		columns.add(new DefExportColumn("bizTodt", Arrays.asList("#cspan", "#cspan", "종료월"), textCenterStyle));
		columns.add(new DefExportColumn("prjtFrdt", Arrays.asList("#cspan", "Project 기간", "시작일"), textCenterStyle));
		columns.add(new DefExportColumn("prjtTodt", Arrays.asList("#cspan", "#cspan", "종료일"), textCenterStyle));
		...
		if(this.option.get("satTrgtYn").equals("Y")) {
			...
			columns.add(new DefExportColumn("firstBizDt", Arrays.asList("#cspan", "최초적용 사업연도 종료월", "#rspan"), textCenterStyle));
			...
			columns.add(new DefExportColumn("sameAdtrSbsidYn", Arrays.asList("#cspan", "동일감사인이 감사하는 종속회사 또는 관계회사 존재 여부", "#rspan"), basicStyle));
			...
			columns.add(new DefExportColumn("riskBase", Arrays.asList("#cspan", "위험계정비중", "비중"), Constant.COLUMN_TYPE.NUMERIC, numberStyle4));
			...
			// 상하한(기준숙련도 기준)
			//======================
			columns.add(new DefExportColumn("minMaxYnNm", Arrays.asList("상하한(기준숙련도 기준)", "상하한 적용", "O/X"), textCenterStyle));
			columns.add(new DefExportColumn("minMaxReason", Arrays.asList("#cspan", "#cspan", "미적용 근거"), basicStyle));
			...
			columns.add(new DefExportColumn("fstAdtTmReason", Arrays.asList("#cspan", "#cspan", "감사 시간 근거"), basicStyle));
			...
			columns.add(new DefExportColumn("fstWkmnspReason", Arrays.asList("#cspan", "#cspan", "감사팀 숙련도 근거"), basicStyle));
			...
			columns.add(new DefExportColumn("priorAdtTmReason", Arrays.asList("#cspan", "#cspan", "전기 감사 시간 근거"), basicStyle));
			...
			columns.add(new DefExportColumn("priorWkmnspReason", Arrays.asList("#cspan", "#cspan", "전기 감사팀 숙련도 근거"), basicStyle));
		...
		columns.add(new DefExportColumn("remark", Arrays.asList("비고", "#rspan", "#rspan"), basicStyle));
	}
	...
	public Workbook generate() throws Exception {
		...
		for(int i=0; i<columns.size(); i++) {
			if(columns.get(i).getCode().indexOf("Reason") < 0) {				
				sheet.autoSizeColumn(i + startColIdx, true); // 자동
			}else {
				sheet.setColumnWidth(i + startColIdx, 10000); // 고정
			}
		}
