...
const projectInfoMixin = {
	...
	computed: {
		...
		totCntrtFee: function(){
			return this.cntrtFee + subPrjt.map(function(data){return Number(data.cntrtFee)}).reduce(function(acc, curr){return acc + curr;}) 
		}
	}
}
...
//			if(!this._checkSelect('desigAdtYn')){msg.push('[지정감사 여부]를 선택하세요.');}
			...
			// 예산 배분
			msg = msg.concat(this._validCheckBdgt());
		...
		_validCheckBdgt: function(){
			let msg = [];
			if(!this._checkField('raBdgtTm')){
				msg.push('[RA 배부 예산]을 입력하세요.')
			}else if(this.raBdgtTm < 0){
				msg.push('[RA 배부 예산]에는 0 이상의 숫자를 입력해야 합니다.')
			}
			if(!this._checkField('flcmBdgtTm')){
				msg.push('[Fulcrum 배부 예산]을 입력하세요.')
			}else if(this.flcmBdgtTm < 0){
				msg.push('[Fulcrum 배부 예산]에는 0 이상의 숫자를 입력해야 합니다.')
			}
			return msg;
		}
	}
