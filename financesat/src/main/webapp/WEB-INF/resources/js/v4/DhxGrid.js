...
//				this.gridObj.refreshFilters();
		...
		addRow: function(list) {
			const thisObj = this;
			const prjtPopup = Helper.Popup("project", "Project Select", "/popup/project-v4", {size:'M', prjtCd:this.options.prjtCd, list: JSON.stringify(list)});
		...
		excelExport: function(param) {
			this.gridObj.toExcel(EXCEL_WRITER_URL_V4 + "?" + $.param(param));
		},
		refreshGridByMode: function(){
			if(this.mode === 'read'){
