			...
			(function(){
				$("#btnDownloadBdgtByEmp").off('click').on('click', function(){
					...
					$.fileDownload("${pageContext.request.contextPath}/excel/export/bdgtByEmp", {
						httpMethod: "POST", 
					    data: $.param({sdt: sdt,edt: edt,filename: "인원별 전체 Budget(" + sdt + "~" + edt + ")"}),
				...
				$("#btnDownloadSatBdgtInfo").off('click').on('click', function(){
					...
					$.fileDownload("${pageContext.request.contextPath}/excel/export/satBdgtInfo", {
						httpMethod: "POST", 
					    data: $.param({baseDt: baseDt,filename: "표준감사시간 Summary(" + baseDt + " 이후)"}),
					    successCallback: function (url) { 
					    	COMMON.hideLoading();
				       },
				       failCallback: function (responseHtml, url, error) {
		...
		<dl class="item">
			...
			<dd>
				<div class="selectbox" style="width:120px">
					<select id="cbo_prfl" title=""></select>
				</div>
			</dd>
		</dl>
		<p class="txtDesc"></p>
...
setTimeout(function(){
	...
	Helper.post("/admin/getProfileList")
	.done(function(){
		$.when(
				Helper.post("/admin/getWkmnsp", {prflId:1}),
				Helper.post("/admin/getFactor", {prflId:1})
			)
			.done(function(wkmnsp, factor){
	...
	function drawTableRate(headers, dataRows){
		if(!dataRows){
			dataRows = headers;
		}else{
			...
			let headerHtml = [];
			let headerObj = [];
			headerHtml.push("<th cd=\"year\">구분</th>");
			headerObj.push({id:"year", text:"구분"});
			for(let i=0; i< headers.length; i++){
				headerHtml.push("<th code=\"" + headers[i].cd + "\">"+ headers[i].nm + "</th>");
				headerObj.push({id:headers[i].cd , nm:headers[i].text});
			}
			
			$('#rate-grid > thead > tr').html(headerHtml.join(""));
			$('#rate-grid').data("header", headerObj);
		}
		
		let rowHtml;
		let headerList = $('#rate-grid').data("header");
		let year = 0;
		for(let i=0; i<dataRows.length; i++){
			let rows = [];
			if(year != dataRows[i].aplyYear){
