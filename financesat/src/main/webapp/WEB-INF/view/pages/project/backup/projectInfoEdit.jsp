...
<c:set var="isReadComponent" value="${auth.cmpltCnt > 0}" />
...
<div id="projectEdit" class="boxWhite">
	<div class="pageFixed">
		<div class="inner">
			<ul class="tabType01">
			...
			<div class="btnArea">
				...
				<button sh-click="cancel" id="btnCancel" type="button" class="btnPwc btnM action">Cancel</button>
			</div>
		</div>
	</div>
	...
	<table class="tblForm">
		...
		<tbody>
			<tr>
				...
				<td colspan="3">
					<div class="searchAreaS">
						...
						<p class="txtInfo">프로젝트 변경은 불가하며, 삭제 후 재 작성해야 합니다.</p>
					</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><div class="frmNowrap"><input sh-name="cntrtFee" sh-type="number" sh-format="#,##0" type="text" class="tright"><span class="unit">원</span></div></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${isReadComponent}">
							<span sh-name="prjtFrdt"></span>
						</c:when>
						<c:otherwise>
							<div class="calenWrap">
	...
	<table  sh-test="  $satTrgtYn != 'Y' " sh-dotype="hide" class="tblForm"  style="display:none">
		...
		<tbody>
			...
			<tr>
				...
				<td class="tdSel">
					<div class="selectbox frmW01">
						<select title="그룹1의 예외사항 여부" sh-select="satgrp1ExptYn" sh-name="satgrp1ExptYn">
							<option value="">선택하세요.</option>
			...
			<tr>
				...
				<td><div class="frmNowrap"><input  sh-type="number" sh-format="#,##0" type="text" class="tright" sh-name="priorAdtTm"><span class="unit">hr</span></div></td>
			</tr>
			<tr>
				<th scope="row">기업규모
					<div class="tipArea">
				...
				<td class="tdSel">
					<div id="satgrp_select_box" class="selectbox frmW01">
						<select title="표준 감사 시간 그룹 선택" sh-name="satgrpCd" sh-dotype="disable" sh-test="$satgrp1ExptYn == 'Y'" sh-select="satgrpCd">
							<option value="">선택하세요.</option>
							<c:forEach var="option" items="${satGrpList}">
							<option value="${option.satgrpCd}">${option.satgrpNm}</option>
							</c:forEach>
						</select>
					</div>
				</td>
			...
			<tr>
				...
				<td><div class="frmNowrap"><input sh-type="number" type="text" name="firstAdtFctr" class="frmW01 tright" sh-name="firstAdtFctr" sh-dotype="hide" sh-test="!($satgrpCd == 'SATGRP01' and $firstAdtYn == 'Y')" style="display:none"></div></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="4" class="tdBtn tright">
					...
					<%-- <a class="btnPwc btnS" href="${pageContext.request.contextPath}/resources/templates/표준감사시간 Formula_20190214(최종안)_배포금지용18.xlsx" download >Simulation File Download</a> --%>
			...
			<tr>
				...
				<td><div class="frmNowrap"><input sh-type="number" sh-format="#,##0" type="text" class="tright" sh-name="etDfnSat"><span class="unit">hr</span></div></td>
			</tr>
			<tr>
	...
	<h2 class="titDep2">표준감사시간외 예산입력</h2>
	<table class="tblForm">
...
$(document).ready(function(){
	...
	setTimeout(function(){
		Helper.post("/project/info", {prjtCd:"${prjtCd}", isDraft:${isDraft}})
		.done(function(data){
			initData =  Object.assign({}, prjtDef, data);
			...
			prjtInfo = new ProjectInfo({
				...
				event:{
					click:{
						goBdgt: function(e){
							...
							<c:choose>
								<c:when test="${isDraft}">
									Helper.MessageBox("먼저 Save 버튼을 클릭하여 프로젝트 정보를 등록하시 바랍니다.");
								</c:when>
								<c:otherwise>
									Helper.ConfirmBox(["프로젝트 정보가 저장되지 않았습니다", "저장하지 않고 Bugeting으로 이동하시겠습니까?"], function(flag){
										if(flag){
											Helper.goPage("/pages/project/budget/edit", {prjtCd: "${prjtCd}", isDraft:${isDraft}});
										}
									});
								</c:otherwise>
							</c:choose>
						...
						save: function(){
							...
							if(!${isDraft} && (initData.prjtFrdt != this.prjtFrdt || initData.prjtTodt != this.prjtTodt)){
								Helper.ConfirmBox(["프로젝트 기간이 변경되었습니다.", "프로젝트 기간이 변경되면 Budgeting의 데이터가 초기화 됩니다.", "계속 하시겠습니까?"], function(flag){
									if(flag){
										Helper.post("/project/update", Object.assign(thisObj, {clearBdgt:"Y"}))
										.done(function(){
											let msg = [];
							...
							} else if(${auth.stat == 'CO' && !(roleCd == 'sysadmin' || auth.isEl) ? true : false}) {
								Helper.ConfirmBox(["저장하면 [결재완료]에서 [등록] 상태로 변경되며 재승인 받으셔야 합니다.", "저장하시겠습니까?"], function(flag){
									if(flag){
										Helper.post("/project/update", Object.assign(thisObj, {reAprv:"Y"}))
										.done(function(){
											let msg = [];
											msg.push("저장 완료 되었습니다.");
											Helper.MessageBox(msg, function(){
								...
								let param = this;
								<c:if test="${isDraft}">
								param = Object.assign(param, {stat:"RG"});
								</c:if>
								Helper.post("/project/update", this)
								.done(function(){
						...
						saveAsDraft: function(){
							...
							Helper.post("/project/update", thisObj)
							.done(function(){
				...
				validation: {
					...
					messages:{
						...
						satTrgtYn:{required: "[표준감사시간 대상 여부 여부]를 선택하세요"},
