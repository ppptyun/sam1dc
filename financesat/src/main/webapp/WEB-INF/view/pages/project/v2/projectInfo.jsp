...
<div id="projectInfo" :class="{ieBody: isIE, boxWhite: true, readmode:isRead}" style="display: none">
	...
	<table class="tblForm">
		...
		<tbody>
			...
			<tr>
				<th scope="row">사업연도 시작월<span class="ness">*</span></th>
				<td>
	...
	new Vue({
		...
		methods:{
			...
			__save: function(){
				...
				if(thisObj.mode === "new"){
					Helper.post("/project/v2/add", param)
					...
					Helper.post("/project/v2/update", param)
					.done(function(){
						Helper.MessageBox("저장 완료 되었습니다.")
						thisObj.mode = 'read'
						thisObj.isDraft = false;
			...
			onDelete: function(){
				...
				if(this.mode === 'read'){
					Helper.ConfirmBox('삭제 하시겠습니까?', function(flag){
						if(flag){							
							Helper.post("/project/v2/delete", {prjtCd: thisObj.prjtCd})
							.done(function(){
			...
			onTest: function(){
				...
				let param = Object.assign({}, this.$data, computed)
				console.log(param)
			},
