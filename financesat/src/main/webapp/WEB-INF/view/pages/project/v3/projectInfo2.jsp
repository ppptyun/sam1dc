...
const computedFields = ['compSize', 'factorVal', 'fstBizDt', 'fstMaxTm', 'wkmnspSat', 'minTm', 'maxTm', 'totCntrtFee', 'totPrjtBdgt', 'calSat']
// 데이터 
function fetchData(){
	const $deferred = $.Deferred();	
	if('${prjtCd}'){
		Helper.post('/project/v3/info', {prjtCd: '${prjtCd}', isDraft: "${isDraft}".bool}).done(function(prjtInfo){$deferred.resolve(prjtInfo);})
	}else{
		$deferred.resolve({});
	}
...
fetchData().done(function(prjtInfo){
	let oData = Object.assign({}, initData, prjtInfo);
	...
	new Vue({
		...
		methods:{
			onLoad: function(prjtInfo) {
				origin = JSON.stringify(prjtInfo);
			...
			onSave: function(){
				// edit 모드일 경우 확인사항
				// 1. 프로젝트 기간 변경 여부 확인 -> 프로젝트 기간 변경시 -> Budget 데이터 모두 삭제
				// 2. 결재 완료된 문서 수정인지 확인 -> 결재완료 문서 수정시 'RG' 상태값으로 변경
			...
			onSatCalc: function(){
				...
				popSatCalc.openPopup("satcalc", function(satCalcData){
					...
					if((thisObj.bizFrdt === null || thisObj.bizFrdt === '') && (thisObj.bizTodt === null || thisObj.bizTodt === '')){
						...
						if(data.bizFrdt === satCalcData.bizFrdt && data.bizTodt === satCalcData.bizTodt){
							Object.assign(data, satCalcData);	
						}else{
			...
			trimSubPrjt: function(){
				let trimSubPrjt = this.subPrjt.filter(function(item){
					return item?item.prjtCd !== null && item.prjtCd !==null && item.prjtCd !==null :false
				});
				if(this.subPrjt.length != trimSubPrjt.length) this.subPrjt = trimSubPrjt; 
			},
