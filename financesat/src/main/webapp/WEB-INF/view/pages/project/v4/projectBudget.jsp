...
<div id="budgetInfo">
	...
	<div class="boxWhite" :class="{ieBody: isIE, boxWhite: true, readmode:mode=='read'}" v-show="isInit" style="display: none">
		<ul class="tabType01">
			...
			<li class="on"><span>Budgeting V4</span></li>
		</ul>
		<div class="btnArea">
		...
		<div class="div-row">
			<div class="div-col">
				<div class="titArea marT20">		
					<h2 class="titDep2">Budget 입력 주기</h2>
				</div>
				<div>
					<selectbox id="budgetInputCycle" v-model="budgetInputCycle" title="Budget 입력 주기">
						<c:forEach var="option" items="${selOption.BUDGET_INPUT_CYCLE}">
						<option value="${option.cd}" ${option.cd == result.data.budgetInputCycle?"selected":""}>${option.nm}</option>
						</c:forEach>
					</selectbox>								    
				</div>
			</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/v4/DhxGrid.js"></script>
<script>
const weekInfo = [];
const workDayList = [];
...
//20230210 budgetInputCycle 추가
const excelImportPopup = function(){return Helper.Popup("excelImport", "Import Excel", "/popup/v4/fileUploader", {size:"S", importUrl:"/excel/import/v4/budgetHours", prjtCd:"${result.data.prjtcd}", allowExtensions:["xlsx", "xls"], budgetInputCycle:"${result.data.budgetInputCycle}"});};
...
const budgetInfo = new Vue({
	...
	data: {
		...
		budgetGridOptions: {},
		budgetInputCycle: {cd:null, nm:null},
	},
	created: function() {
		const thisObj = this;
		// fetch Data
...
//		console.log('budgetInputCycle : ' + this.data.budgetInputCycle);
		
		this.budgetInputCycle.cd = this.data.budgetInputCycle;
		this.budgetInputCycle.nm = $("#budgetInputCycle option:checked").text();
//		console.log('budgetInputCycle option:checked : ' + $("#budgetInputCycle option:checked").text());
	...
	methods: {
		loadData: function() {
			let dfd = $.Deferred();
			const thisObj = this;
			Helper.post("/project/v4/budget/summary", {
		...
		loadCisData: function() {
			...
			const thisObj = this;
			$.promise
			Helper.post("/project/v4/budget/cislist", {
				prjtCd: this.data.prjtcd
			}, true)
			.fail(function(){
				dfd.reject();
		...
		loadBudgetData: function() {
			let dfd = $.Deferred();
			const thisObj = this;
			Helper.post("/project/v4/budget/list", {
				prjtCd: this.data.prjtcd
			   ,budgetInputCycle: this.data.budgetInputCycle	
			}, true)
			.fail(function(){
				dfd.reject();
			})
		...
		onExcelImport: function() {
			if("${result.data.profBdgtYn}" === "Y") {
				...
				if(this.data.stat === 'CO') {
					Helper.ConfirmBox(['결재완료된 프로젝트입니다.', '저장하면 [결재완료]에서 [등록]상태로 변경되며 재승인 받으셔야 합니다.', '계속 하시겠습니까?'], function(flag){
						if(flag) {							
		...
		onExcelExport: function() {
			Helper.ConfirmBox(["엑셀 다운로드 하시겠습니까?", "[참고] Import 목적으로 다운로드 시 Activity ~ Total Time 까지의 컬럼은 수정하지 마세요."], function(isOk){
				let param = {
					...
					writerType: "budget",
					budgetInputCycle: "${result.data.budgetInputCycle}"
				};
...
//				console.log(this.data.profBdgtReason)
		...
		onSave: function() {
			...
			const param = {
				  prjtCd:"${result.data.prjtcd}"
				, list: budgetList
				, cislist: thisObj.cisData
				, stat: thisObj.data.stat
				, budgetInputCycle: thisObj.budgetInputCycle.cd
			};
			...
			if(msg.length > 0) {
				Helper.ConfirmBox(msg.concat('계속 하시겠습니까?'), function(flag){
					if(flag) {
						Helper.showLoading();
						Helper.post("/project/v4/budget/save", param)
						.done(function(){
							<c:choose>
								<c:when test="${result.data.returnUrl == '/pages/approval/details'}">
									Helper.goPage("/pages/approval/details");
								</c:when>
								<c:otherwise>
									Helper.goPage("/pages/project/budget/read", {prjtCd:"${result.data.prjtcd}"});
								</c:otherwise>
		...
		onRequestApproval: function() {
			if(this.data.aprvCmpltDt !== null) {
				...
				reasonModifyPop().openPopup('reasonModifyPop', function(data) {
					...
					Helper.post("/project/v4/budget/reqELAprv", data)
					.done(function(){
						Helper.goPage("/pages/project/budget/read", {prjtCd: "${result.data.prjtcd}"});
					});
				})
				...
				Helper.ConfirmBox("EL 승인 요청을 하시겠습니까?", function(flag){
					if(flag){						
						Helper.post("/project/v4/budget/reqELAprv", {prjtCd:"${result.data.prjtcd}"})
						.done(function(){
							Helper.goPage("/pages/project/budget/read", {prjtCd: "${result.data.prjtcd}"});
						});
					}
		...
		validCheck: function() {
			...
			if (bdgtTrgtlist.length > 0) {
				// 1. Budget 대상으로 지정된 프로젝트 코드가 중복인지 확인
				const retData = Helper.postSync('/project/v4/budget/checkDuplPrjt', {mainPrjtCd: this.data.prjtcd ,prjtList: bdgtTrgtlist});	
	...
	watch:{
		budgetInputCycle: function(){
			var budgetInputCycleCd = this.budgetInputCycle.cd;

			//alert(this.mode);			
			//alert(this.budgetInputCycle.cd);
			//alert("${result.data.budgetInputCycle}");
			
			//1.이전 값과 비교해서 변화가 이루어지면 수행한다.
			if(this.budgetInputCycle.cd != "${result.data.budgetInputCycle}"){
				if(this.mode == 'edit'){
					Helper.ConfirmBox(['Budget 입력주기가 변경되면 화면이 새로고침 됩니다.', '저장하지 않은 내용은 초기화 됩니다.', '계속 하시겠습니까?'], function(flag){
						if(flag) {							
							Helper.goPage("/pages/project/budget/edit", {prjtCd: "${result.data.prjtcd}", budgetInputCycle: budgetInputCycleCd}); //여기 안쪽에서는 this.budgetInputCycle.cd 값이 인식이 안된다. 밖에서 변수에 담아서 사용함.
						}
					});
				}
				else{
					//read
					Helper.goPage("/pages/project/budget/read", {prjtCd: "${result.data.prjtcd}", budgetInputCycle: this.budgetInputCycle.cd});
				}
			}
		},
	},
