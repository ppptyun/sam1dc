	...
	<div class="pageFixed">
		<div class="inner">
			...
			<c:if test="${result.data.stat == 'RJ'}">
				<button sh-click="btnRjReason" type="button" class="btnPwc btnM action">반려의견 보기</button>
			</c:if>		
			<c:if test="${result.data.canEdit && (result.data.stat == 'RQ') }">
				<button sh-click="btnApprove" type="button" class="btnPwc btnM action">승인</button>
				<button sh-click="btnReject" type="button" class="btnPwc btnM action">반려</button>
			</c:if>	
			<button sh-click="btnClosePrjInfo" type="button" class="btnPwc btnM action">Close</button>
			</div>
		</div>
	</div>
	...
	<table  sh-test="  $satTrgtYn != 'Y' " sh-dotype="hide"  style="display:none"  class="tblForm marT40 lineT">
		...
		<tbody>
			...
			<tr>
				...
				<td class="tright"><span sh-name="etTrgtAdtTm" sh-format="#,##0.00"></span> hr</td>
...
const rejectOpinionPopupInput = Helper.Popup("rjOpinion", "반려의견 등록", "/popup/rjopinion", {prjtCd:"${result.data.prjtCd}"});
			...
			prjtInfo = new ProjectInfo({
				...
				event:{
					click:{
						goBdgt: function(e){
							e.preventDefault();
							Helper.goPage("/pages/approval/details", {prjtCd:"${result.data.prjtCd}"});
						},
						btnApprove: function(){
						...
						btnClosePrjInfo: function(){
							Helper.goPage("/pages/approval/list");
						},
						btnRjReason: function(){
